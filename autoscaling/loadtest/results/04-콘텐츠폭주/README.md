...
## 한눈에 보기 - 같은 시험 10회, 처리량 +100%의 여정

이 문서는 **같은 부하(500 VU·6분)를 구성만 바꿔가며 10번 돌린 기록**이다. 무엇이 통했고
무엇이 반증됐는지가 처리량 하나로 요약된다:

```mermaid
xychart-beta
    title "T2-B 서버 처리량 진화 (rps) - 같은 부하, 구성만 변경"
    x-axis ["baseline 08-17", "limit 1.7 (반증)", "limit 제거", "힙 512", "readOnly 코드", "스팟 이주", "replica 3", "캐시", "스레드 200"]
    y-axis "서버 rps" 0 --> 400
    bar [176.7, 148.8, 180.8, 181.6, 195.6, 241.2, 314.3, 305.1, 353.3]
```

```mermaid
xychart-beta
    title "요청당 CPU 단가 (ms) - 낮을수록 좋음"
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3", "캐시", "스레드 200"]
    y-axis "ms/요청" 0 --> 25
    line [16.9, 22.6, 20.9, 20.5, 18.6, 16.8, 16.2, 16.7, 16.0]
```
...
xychart-beta
    ...
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3", "캐시", "스레드 200"]
    y-axis "%" 0 --> 0.5
    line [0.00, 0.36, 0.32, 0.32, 0.25, 0.03, 0.15, 0.18, 0.06]
```
...
읽는 법: 실패는 10회 내내 **전부 60s 클라 타임아웃류**(5xx 없음)이고, 실패율이 가장 나빴던
회차(#2)가 CFS 동결, 가장 좋았던 회차(#10)가 스레드 상향이다. content 힙 사용은 어떤 구성에서도
