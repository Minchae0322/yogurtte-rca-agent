...
## 한눈에 보기 - 같은 시험 9회, 처리량 +73%의 여정

이 문서는 **같은 부하(500 VU·6분)를 구성만 바꿔가며 9번 돌린 기록**이다. 무엇이 통했고
무엇이 반증됐는지가 처리량 하나로 요약된다:

```mermaid
xychart-beta
    title "T2-B 서버 처리량 진화 (rps) - 같은 부하, 구성만 변경"
    x-axis ["baseline 08-17", "limit 1.7 (반증)", "limit 제거", "힙 512", "readOnly 코드", "스팟 이주", "replica 3", "캐시"]
    y-axis "서버 rps" 0 --> 350
    bar [176.7, 148.8, 180.8, 181.6, 195.6, 241.2, 314.3, 305.1]
```

```mermaid
xychart-beta
    title "요청당 CPU 단가 (ms) - 낮을수록 좋음"
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3", "캐시"]
    y-axis "ms/요청" 0 --> 25
    line [16.9, 22.6, 20.9, 20.5, 18.6, 16.8, 16.2, 16.7]
```
...
xychart-beta
    ...
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3", "캐시"]
    y-axis "%" 0 --> 0.5
    line [0.00, 0.36, 0.32, 0.32, 0.25, 0.03, 0.15, 0.18]
```
