...
## 한눈에 보기 - 같은 시험 8회, 처리량 +61%의 여정

이 문서는 **같은 부하(500 VU·6분)를 구성만 바꿔가며 8번 돌린 기록**이다. 무엇이 통했고
무엇이 반증됐는지가 처리량 하나로 요약된다:

```mermaid
xychart-beta
    title "T2-B 서버 처리량 진화 (rps) - 같은 부하, 구성만 변경"
    x-axis ["baseline 08-17", "limit 1.7 (반증)", "limit 제거", "힙 512", "readOnly 코드", "스팟 이주", "replica 3"]
    y-axis "서버 rps" 0 --> 350
    bar [176.7, 148.8, 180.8, 181.6, 195.6, 241.2, 314.3]
```

```mermaid
xychart-beta
    title "요청당 CPU 단가 (ms) - 낮을수록 좋음"
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3"]
    y-axis "ms/요청" 0 --> 25
    line [16.9, 22.6, 20.9, 20.5, 18.6, 16.8, 16.2]
```

```mermaid
xychart-beta
    title "실패율 (%) - 전부 60s 클라 타임아웃류, 5xx 아님"
    x-axis ["baseline", "limit 1.7", "limit 제거", "힙 512", "readOnly", "스팟 이주", "replica 3"]
    y-axis "%" 0 --> 0.5
    line [0.00, 0.36, 0.32, 0.32, 0.25, 0.03, 0.15]
```
...
읽는 법: 실패는 8회 내내 **전부 60s 클라 타임아웃류**(5xx 없음)이고, 실패율이 가장 나빴던
회차(#2)가 CFS 동결, 가장 좋았던 회차(#6)가 노드 분리다. content 힙 사용은 어떤 구성에서도
196~361Mi라 메모리는 한 번도 병목이 아니었고, 병목은 CPU(1~5회) → 노드(6·7회) → **DB 커넥션(8회~)**
순으로 이동했다.
...
## 재실행 9 (2026-08-23) - 핫리스트 캐시 After: 적중 100%인데 T2-B는 등가 - 무대가 달랐다

캐시(앱개선 C-2, toy-content `4353753`) 배포 후 동일 조건. **적중 hit 95.8/s·miss 0** ·
hot med 82→**37ms** · 전체 med 93ms. 그러나 서버 rps 305.1(등가)·단가 16.7ms·RDS 59 -
**예측(~500rps·단가 11ms·RDS 하락) 반증.** 원인: T2-B의 비싼 경로(scroll·detail)는 실시간이라
캐시 제외 대상이고 hot은 원래 싼 경로 + RDS 59는 풀 상주분(minIdle). 상세 해부는
[앱개선.md](../../../개선/앱개선.md) C-2. 캐시의 본 무대(실트래픽 구성) 검증은 journey-v2 After로 남음.
