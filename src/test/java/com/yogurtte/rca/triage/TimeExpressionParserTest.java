...
class TimeExpressionParserTest {
    ...
    void 절대_시각을_최근N분으로_오독하지_않는다() {
        ...
        var afternoon = Instant.parse("2026-07-28T07:00:00Z");

        var resolved = parser.resolve("14시 20분쯤에 앱이 버벅였다는 제보가 있어요", null, null, afternoon);
    ...
    void 어제_새벽은_오늘로_밀리지_않는다() {
        ...
        var resolved = parser.resolve("어제 새벽에 앱이 이상했대요", null, null, NOW);
    ...
    @Test
    void 날짜와_시각이_결합된다() {
        var resolved = parser.resolve("어제 14시 20분쯤 앱이 버벅였어요", null, null, NOW);
    ...
    void 날짜_없는_미래_시각은_가장_가까운_과거로_해석한다() {
        ...
        var resolved = parser.resolve("14시 20분쯤 버벅였다는 얘기가 있어요", null, null, NOW);
    ...
    void 시각_구간을_읽는다() {
        ...
        var resolved = parser.resolve("10시부터 11시까지 로그인이 느렸어요", null, null, afternoon);
    ...
    void 시간대_단독이_미래면_어제로_해석한다() {
        ...
        var resolved = parser.resolve("저녁에 앱이 이상했어요", null, null, NOW);
    ...
    @Test
    void 접미_표지도_상대_표현이다() {
        var resolved = parser.resolve("5분 전에 이상했어요", null, null, NOW);
    ...
    void 미래_to는_now로_잘린다() {
        var resolved = parser.resolve("아무 질문", Instant.parse("2026-07-28T04:00:00Z"),
