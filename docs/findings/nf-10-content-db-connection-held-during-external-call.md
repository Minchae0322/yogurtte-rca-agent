...
> 아니어야** 한다. 그리고 AU-1(auth CPU 기아)을 주입했을 때 `hikaricp_connections_pending`이
> 0을 유지해야 한다 — 현재 구조라면 pending이 쌓인다.
> **반증 조건**: 분리 후에도 AU-1에서 pending이 쌓이면 병목은 다른 곳이다.
...
- **NF-01이 단발 사례가 아님을 보인다.** chat 컨슈머(쓰기 경로)에서 발견한 안티패턴이
  ...
  같은 기준으로 훑어볼 근거가 된다.
- **AU-1의 사전 가설이 된다.** AU-1(auth CPU 기아)은 아직 미실행인데, 이 구조 때문에
