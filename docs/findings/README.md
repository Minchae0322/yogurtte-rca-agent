...
| [AE-06](ae-06-rca-v0-au4-blind-eval.md) | rca-agent v0 × AU-4 파일럿 — **앵커가 틀리고 에이전트가 맞았다**(3s timeout이 아니라 23.5ms RST) → v3 앵커 도출 | 도구 | 파일럿 채점 불가(70/70) → **재실행 회차 1은 v3로 75/100** |
| [AE-07](ae-07-rca-v0-ap2-round2-eval.md) | rca-agent × AP-2 회차 2(자연어) — **도구 결함 2건 신규**: ③ 라인 필터가 스택트레이스를 배제(NPE 원문 미도달) · ④ traceId 미선정 시 **Tempo 통째로 skip**(트레이스는 있었다). 예측 델타 +25가 **−10으로 반증** | 도구 | **§8 채점 53/100** (N=1, 인용 불가) · **루브릭 v3 첫 적용** |
