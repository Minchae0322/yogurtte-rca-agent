...
| **2 · 2차 실행** | 07-29 11:34:49~11:47:43 | **12분 54초** | **A** | 동일 — auth 호출 **7.76ms refused** + 응답 **200** · 작성자 **"운영자" → "사용자1"**. baseline은 auth 호출 **88.26ms 성공 + Redis `SET ×4`** (**83/100**) | O | [round-2-run2.md](round-2-run2.md) |
...
> **🏆 회차 2 · 2차 실행 결과 (2026-07-29)**: **83 / 100** — 같은 문안·앵커·도구.
> **§8.1 N=2 성립: 평균 85.5 ± 2.5 (인용 가능).**
>
> **다섯 항목이 편차 0**(원인 30 · 근거 25 · 탐색 15 · 오귀인 3 · 조치 5)이고 갈린 하나가
> **4) 영향(10 ↔ 5)** 이다 — **AP-3 쌍과 구조가 완전히 같다.** 실행 1의 4)도 그때 ⚠️ 판정이었고
> *"반대 판정이면 83"* 이라고 적혀 있었으므로, **판정을 통일하면 편차는 0이고 밴드는 83~88**이다.
> → 분산의 출처가 **앵커의 중간 구간 부재**라는 주장이 두 번째 문항에서 재현됐다.
>
> **🔴 1) 원인 30이 세 번째다** — 회차 1(traceId·4항목) · 실행 1 · 실행 2 모두 30이고 사유도
> *"캐시 만료라는 발현 조건 미언급"* 로 같다. 출력이 2,980 → 7,982 → **10,342**로 3.5배 폭
> 움직였는데도 그대로다. **분량 가설은 두 번 반증됐고 남은 레버는 프롬프트뿐이다.**
>
> **채점자 실측**: 캐시 만료의 직접 증거가 창 안에 있었다 — baseline 트레이스(주입 **10초 전**)에
> Redis **`SET ×4`**, TTL 10분 만료가 11:44:39, 첫 증상이 **11:45:52**다. 결함 ④(선정 1건만 수집)로
> 에이전트에게 도달하지 못했다. 상세는 [round-2-run2.md](round-2-run2.md).
