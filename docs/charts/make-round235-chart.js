 ...
 *
 * 시리즈 색은 회차 1 파랑 · 회차 2 주황 · 회차 3 청록 — 3색 조합은 CVD ΔE 9.2로 검증됐고,
...
// s = 회차별 총점, t = 회차별 토큰(회차 1·2·3 순). null = 측정 불가(AU-3 회차 1 미실행).
// § = 개정 앵커 적용값 · † = 분석 단계만 기재 — 라벨 문자열에 그대로 싣는다.
// 문항 ID는 출처 추적용으로만 둔다 - 차트에는 그리지 않는다.
// 순서는 회차 1 총점 내림차순 — round-2 차트와 같은 자리여야 옆에 놓고 대조가 된다.
const ITEMS = [
...
const SERIES = [
  { label: '회차 1',             color: '#2a78d6', overhead: 48 },
  { label: '회차 2 (도구 개선)', color: '#eb6834', overhead: 48 },
  { label: '회차 3 (접기·압축)', color: '#1baf7a', overhead: 59 },
];
const C = {
...
panel(B_TOP,
  '(b) 회차 1·2·3 입력 토큰 - 문항별 실행 평균',
  '조사 1회에 LLM으로 들어간 입력량. 색은 실제 관측 데이터, 회색은 어느 조사에나 붙는 CLI 고정비(회차 1·2 약 48 · 회차 3 약 59) · 숫자는 색 부분만 (단위 1,000 토큰)',
  (item, se, k, cx, base) => {
    if (item.t[k] == null) {
...
push(text(PAD.left, H - 56,
  '회차 1→2→3은 같은 자로 채점됐으나 도구 세대·조사 창·주입이 회차마다 달라 문항별 델타의 귀속은 회차 문서를 따른다 · § 개정 앵커 적용값 · † 분석 단계만 기재',
  { size: 16, fill: C.faint, anchor: 'start' }));
push(text(PAD.left, H - 30,
  'AU-3 회차 1은 조사 미실행(0점 · 토큰 측정 불가) · 회차 3 CH-1 토큰은 실행 2뿐 · AU-2 회차 1은 편차 ±11로 §8.1 인용 보류 · 토큰 절감 인용은 AP-1 −60.5% · AP-2 −61.9% · AP-3 −49.7% · AU-4 −48.5% · AU-2 −39.0%(분석)만',
  { size: 16, fill: C.faint, anchor: 'start' }));
push('</svg>');
