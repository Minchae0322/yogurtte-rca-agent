# CH-1 이관 X (번호 반납) — 다운 73초: 경계 안쪽, 지연으로 흡수

> **2026-07-29 재번호로 회차 번호를 반납했다.** [갈래 분리](../scoring/README.md#ch-1-갈래-분리--ch-3-신설-2026-07-28)로
> 이 회차의 전개(대기 중 복구 → 첫 시도 성공, 재시도·DLQ 미발동)는 **CH-3 문항**이 됐다.
> CH-1은 갈래 B(다운 ≥5분 → DLQ)만 남으므로, 이 회차는 CH-1의 회차로 세지 않고
> **[CH-3](../ch-3/README.md)의 사료**로만 쓴다. 점수·본문은 **무수정 보존**(§8.2)이며
> 파일명만 `round-1.md` → `round-x-73s.md`로 바뀌었다.
...
| **§8 채점** | **채점 불가 (앵커 부적합)** — 근본 20/40 · 오귀인 20/20은 산출됐으나, 근거 경로·조치는 앵커가 "재시도→DLQ" 전개를 전제해 **이 회차엔 발생하지 않은 신호**를 만점 요건으로 요구한다. 사유·처리는 [채점 대장](../scoring/README.md#ch-1-이관-x-구-회차-1--채점-불가-앵커-부적합) |
| **토큰·비용·시간** | in 44,798 / out 9,162 tok · **$1.0845** · 135.4s |
| **에이전트 보고서 전문** | [round-x-73s-rca-report.md](round-x-73s-rca-report.md) |
...
![업스트림은 전부 정상 — content POST 72.75ms, notification-publish 4.64ms, publish 18ms. 붉은 24.7초는 chat receive에서 시작된다](roundx-73s-trace-upstream.png)
...
![process-notification 24.69초 막대 안에서 자식 span(mongo insert 21.24ms 등)이 전부 오른쪽 끝에 몰려 있다 — 왼쪽의 빈 구간이 23.4초 공백](roundx-73s-trace-gap.png)
...
- RCA 리포트: [round-x-73s-rca-report.md](round-x-73s-rca-report.md) (원본 `reports/6a65bd43…-20260726T080237.md`는 gitignore) — in 44,798 / out 9,162 tok · $1.0845 · 135.4s
