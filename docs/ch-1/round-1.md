# CH-1 회차 1 — 다운 4.5분: 경계 바깥, DLQ 경유 복구

> **2026-07-29 재번호: 구 회차 2 → 회차 1.** [갈래 분리](../scoring/README.md#ch-1-갈래-분리--ch-3-신설-2026-07-28)로
> 73초 회차가 [CH-3 사료](round-x-73s.md)로 이관되면서, CH-1(갈래 B)의 첫 회차는 이 회차가 됐다.
> `anchors-v2.md`도 이 회차를 *"회차 1(v0, 80점)"* 으로 부른다. **점수·본문은 무수정 보존**(§8.2)이고
> 파일명만 `round-2.md` → `round-1.md`로 바뀌었다. (이 문서는 구 회차 3이기도 하다 — 2026-07-27 1차 재번호.)
...
| **§8 채점** | **80 / 100** (근본 20/40 · 근거 30/30 · 오귀인 20/20 · 조치 10/10) — 감점 전액이 유실 오판. 근거·판정은 [채점 대장](../scoring/README.md#ch-1-회차-1-구-회차-2--80--100). **N=1이라 §8.1상 인용 불가** |
...
![content POST 44.91ms와 publish 14.47ms는 정상, 첫 receive가 30.07초 에러로 시작된다](round1-trace-fail-first.png)
...
![30초 에러 receive 4개가 계단식으로 이어지고 마지막에 publish user.notifications.dlq 687ms](round1-trace-retries-dlq.png)
...
![같은 경로의 정상 처리 — receive 697.82ms에 전부 끝난다](round1-trace-baseline.png)
![정상 처리 내부 — mongo insert 1.53ms, push-dispatcher 606ms](round1-trace-baseline-children.png)
...
- RCA 리포트: [round-1-rca-report.md](round-1-rca-report.md) (원본 `reports/6a65c38b…-20260726T082912.md`는 gitignore) — in 47,503 / out 6,756 tok · $1.0200 · 101.8s
