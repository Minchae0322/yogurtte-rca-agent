...
========|=================== 탐색 TriageService ===========================
  ...
  [E] SurveyContextAssembler    후보 + 채널요약 + 무신호 + 원본JSON     |
      include-raw=true (현재 기본) -> 원본이 이 컨텍스트의 약 94%       |
        |                                                              |
        v                                                              |
...
**탐색 LLM이 받는 것**: 후보 목록 + 채널별 도달 요약 + 무신호/실패 목록 + (선택) 스윕 원본 JSON
...
### I. ContextAssembler — 가공하지 않는다

6개 절을 그냥 이어붙인다. v0는 **모델에게 원본을 주고 추론을 맡긴다.**

```
# 조사 대상          질문 · 창
# 수집 실패/누락      ← 공백을 근거로 쓰라고 알린다
# 호출 그래프         ← 유일한 "요약" (원본을 대체하지 않고 더한다)
# 트레이스 (Tempo)    ← 원본 JSON, N건
# 로그 ERROR/WARN     ← 원본 JSON
# 로그 traceId 일치   ← 중복 접기(B-10) 적용
# 메트릭 (Mimir)      ← metric-summary=true면 요약, false면 원본
```
...
메트릭만 이미 요약(B-25)이 걸려 있고, **가장 큰 둘은 원본 그대로 들어간다.**
...
| `rca.collect.metric-summary` | **true** | 메트릭 원본 → 요약 | **적용 완료.** 저장 원본 −64.5% 실측 |
| `rca.survey.include-raw` | **true** | 탐색 원본 JSON 포함 여부 | 원본이 탐색 컨텍스트의 **약 94%**인데 선택 정확도 기여는 **미측정** |
| `rca.survey.trace-limit` | 20 | 스윕 후보 수 | 낮추면 정답이 후보에서 빠진다 |
| `TimeExpressionParser` 실패 | **24시간** | **탐색 비용 $0.45 → $2.66** | B-31 |

### 아직 안 재 본 것

1. **`include-raw=false`** — 탐색 원본을 빼도 후보 선택이 같은가. 실측 **45,991자 → 7,697자(−83%)**.
   **아직 안 뺐다 — 뺄지 말지가 아니라 순서 때문이다.** 회차 3이 INC-1·2를 기각한 인용문이
   *"**13개 샘플** 전 구간 0"* 인데 후보 설명은 `"lag 가 0이었다 (from ~ to)"` 로 **샘플 수가 없다** —
   **기각 품질이 원본에 기대고 있었다.** 그래서 ① 원본에서만 얻던 재료를 요약으로 옮기고
   ② 저장 스윕 재생으로 A/B 탐색만 대조한 뒤 ③ 기본값을 바꾼다.
   지금 빼면 나빠져도 **원본 부재 탓인지 후보 지문 부실 탓인지 갈리지 않는다.**
   → [round-4 §탐색 입력의 원본 94%](round-4/README.md)
