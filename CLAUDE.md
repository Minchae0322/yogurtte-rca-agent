...
### 채점표는 **한 줄 요약 + 항목별 3단**으로 쓴다 — 표 칸에 근거를 몰아넣지 말 것
...
| **이번 리포트** | `요건 \| ✅/❌/⚠️ \| 리포트가 쓴 것` 표. **요건 하나에 한 줄**, 인용은 리포트 원문 |
| **→ N점.** | 한두 문장. **어느 문언과 갈렸는지**를 밝힌다 (예: *"18점 문언과 갈리는 지점이 ⓓ인데 그것을 했다"*) |

**왜 이렇게 쓰나** — 표 한 칸에 근거를 다 넣으면 **재계산이 불가능**해진다.
채점은 나중에 뒤집힐 수 있고(CH-2 회차 2에서 실제로 뒤집혔다), 그때 **어느 요건에서
갈렸는지**를 짚으려면 요건이 행으로 분리돼 있어야 한다.

- **판정(⚠️)이 있으면 그 항목 절에서 반대 판정 시 총점을 밝힌다.**
- **앵커의 산문과 채점 문언을 구별한다.** 난도·배경 설명을 요건으로 읽으면 앵커에 없는
  잣대가 들어가고 그 회차가 §8상 무효가 된다 (CH-2 회차 2 정정 사례).
- **못 받은 신호는 채점자가 직접 쿼리해 존재를 확인하고 기록한다.** "도구가 막았다"는
  추측이 아니라 실측이어야 한다.
